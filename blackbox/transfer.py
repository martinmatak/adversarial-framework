...
# this will be cleverhans.utils_tf
...
def prep_bbox(sess, x, x_test, y_test):
    """
    Load a model that simulates the "remote"
    ...
    :param batch_size: size of testing batches
    :return:
    """
    ...
    print("Evaluating loaded model on legit dataset.")
    evaluate(model, x_test, y_test, 32)
    wrap = KerasModelWrapper(model)

    predictions = wrap.get_logits(x)
    print("Black box model loaded.")
...
def mnist_blackbox(train_start=0, train_end=60000, test_start=0,
    ...
    prep_bbox_out = prep_bbox(sess, x, y, x_test, y_test, BATCH_SIZE)
